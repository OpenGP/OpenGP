...
protected:
    ...
    std::function<void()> user_update_fn = [](){};
...
public:
    ...
    /// @brief set a custom update function to be called every frame
    void set_user_update_fn(std::function<void()> user_update_fn) {
        this->user_update_fn = user_update_fn;
    }
