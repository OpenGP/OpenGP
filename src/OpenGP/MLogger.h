...
public:
    enum WriterFlag{space,nospace,fatal,nofatal,newline,nonewline};
public:
    ...
    inline MLogger& operator<<(const WriterFlag& flag){
        switch(flag){
            ...
            case nonewline: _with_newline=false; break;
            case newline: _with_newline=true; break;
        }
        return *this;
    }
...
#ifdef WITH_EIGEN
namespace Eigen{
    /// HACK: 3D column vector shown orizontally
    inline MLogger operator<<(MLogger m, Eigen::Vector3f& v){
        printf("[%2.2f %2.2f %2.2f]", v[0], v[1], v[2]);
        m << MLogger::nonewline;
        // m << MLogger::nospace << v.transpose();
        return m;
    }
    /// HACK: 3D column vector shown orizontally
    inline MLogger operator<<(MLogger m, Eigen::Vector2f& v){
        printf("[%2.2f %2.2f]", v[0], v[1]);
        m << MLogger::nonewline;
        // m << MLogger::nospace << v.transpose();
        return m;
    }
