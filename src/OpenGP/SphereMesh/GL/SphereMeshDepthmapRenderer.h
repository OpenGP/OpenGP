...
class SphereMeshDepthmapRenderer : public DepthmapRenderer {
private:
    ...
    const GLchar* vshader = R"GLSL(
        ...
        out vec3 fpos;    ///< per-fragment normal
        ...
        void main(){
            ...
            vec4 vpos = V * M * vec4(vpos_prime, 1.0);
...
public:
    ...
    SphereMeshDepthmapRenderer(const SphereMesh &mesh) : mesh(mesh) {
    ...
    void draw() {
        ...
        auto draw_sphere = [&](SphereMesh::Vertex v){
            ...
            shader.set_uniform("ratio", 1.0f);
            shader.set_uniform("vweight", 0.0f);
        ...
        auto draw_cone = [&](SphereMesh::Vertex v0, SphereMesh::Vertex v1) {
            ...
            shader.set_uniform("M", M);
            shader.set_uniform("ratio", ratio);
            shader.set_uniform("vweight", 0.0f);
        ...
        triangle.set_attributes(shader);
        ...
        auto draw_triangles = [&](SphereMesh::Vertex v0, SphereMesh::Vertex v1, SphereMesh::Vertex v2) {
            ...
            shader.set_uniform("M", M);
            shader.set_uniform("ratio", 1.0f);
            shader.set_uniform("v0", t0);
            shader.set_uniform("v1", t1);
            shader.set_uniform("v2", t2);
            shader.set_uniform("vweight", 1.0f);
